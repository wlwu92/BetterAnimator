#!/bin/bash
# Copy from third_party/sapiens/pose/demo/torchscript/pose_keypoints133.sh

SAPIENS_CHECKPOINT_ROOT="$(pwd)/models/sapiens/sapiens_lite_host"
...
for ((i=0; i<TOTAL_JOBS; i++)); do
  GPU_ID=$((i % TOTAL_GPUS))
  cd third_party/sapiens/lite
  CUDA_VISIBLE_DEVICES=${VALID_GPU_IDS[GPU_ID]} python -c "import experiments.hack_registry; exec(open('${RUN_FILE}').read())" \
    ${CHECKPOINT} \
    --num_keypoints 133 \
